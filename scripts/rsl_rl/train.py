...
from rsl_rl.runners import OnPolicyRunner as BaseRunner
...
def main(
    ...
    # print critic-only (privileged) observation terms once
    obs_mgr = getattr(env.unwrapped, "observation_manager", None)
    if obs_mgr is not None and hasattr(obs_mgr, "active_terms"):
        try:
            policy_terms = set(obs_mgr.active_terms.get("policy", []))
            critic_terms = set(obs_mgr.active_terms.get("critic", []))
            privileged_only = sorted(critic_terms - policy_terms)
            print(f"[INFO] critic-only (privileged) obs terms: {privileged_only}")
        except Exception:
            pass
    ...
    use_cat = bool(getattr(agent_cfg, "use_cat", False))
    Runner = BaseRunner
    if use_cat:
        from cat_envs.rl.cat_rsl.runner_cat import OnPolicyRunnerCaT as Runner
    ...
    if isinstance(obs_groups, dict) and "policy" in available_obs:
        ...
        if isinstance(critic_groups, list) and any(group not in available_obs for group in critic_groups):
            ...
            train_cfg["obs_groups"] = obs_groups
    train_cfg["use_cat"] = use_cat
    # create runner from rsl-rl
    runner = Runner(env, train_cfg, log_dir=log_dir, device=agent_cfg.device)
