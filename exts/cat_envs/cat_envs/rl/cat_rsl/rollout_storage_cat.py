...
# All rights reserved.
# path: source/relic/relic/rl/cat_rsl/rollout_storage_cat.py
...
class RolloutStorageCaT:
    ...
    def __init__(
        ...
        self.deltas = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)
    ...
    def add_transitions(self, transition: Transition):
        ...
        deltas = transition.deltas if transition.deltas is not None else transition.dones.float()
        self.deltas[self.step].copy_(deltas.view(-1, 1))
    ...
    def compute_returns(self, last_values, gamma, lam, normalize_advantage: bool = True):
        ...
        for step in reversed(range(self.num_transitions_per_env)):
            ...
            delta = torch.clamp(self.deltas[step], 0.0, 1.0)
            next_is_not_terminal = 1.0 - delta
