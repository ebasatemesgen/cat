...
# All rights reserved.
# path: source/relic/relic/rl/cat_rsl/runner_cat.py
...
class OnPolicyRunnerCaT:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        self.use_cat = bool(self.cfg.get("use_cat", False))
        self.device = device
        self.env = env
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    if self.log_dir is not None:
                        ...
                        if self.alg.rnd:
                            intrinsic_for_log = (
                                intrinsic_rewards.squeeze(-1) if intrinsic_rewards.dim() > 1 else intrinsic_rewards
                            )  # type: ignore[union-attr]
                            cur_ereward_sum += rewards_for_log
                            cur_ireward_sum += intrinsic_for_log  # type: ignore[operator]
                            cur_reward_sum += rewards_for_log + intrinsic_for_log  # type: ignore[operator]
                        else:
                            cur_reward_sum += rewards_for_log
                        # Update episode length
                        cur_episode_length += 1
                        # Clear data for completed episodes
                        # -- common
                        done_flags = dones.squeeze(-1) if dones.dim() > 1 else dones
                        new_ids = (done_flags >= 1.0).nonzero(as_tuple=False).squeeze(-1)
    ...
    def _construct_algorithm(self, obs) -> PPO:
        ...
        if self.use_cat:
            if alg_class_name != "PPO":
                raise ValueError(f"CaT runner only supports PPO. Got '{alg_class_name}'.")
            alg_class = PPOCaT
        else:
